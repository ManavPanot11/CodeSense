...
/**
 * Execute Python code via OpenRouter LLM as a code interpreter fallback.
 * Asks the model to act as a Python runtime and return exact stdout/stderr.
 */
async function executePythonViaLLM(
  code: string
  ...
  if (!apiKey) {
    return {
      stdout: "",
      stderr:
        "Python is not installed locally and no OPENROUTER_API_KEY is set.\n\n" +
        "To run Python code, either:\n" +
    ...
    "https://openrouter.ai/api/v1/chat/completions",
    {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a Python code execution engine. Execute the given Python code EXACTLY as a Python interpreter would. Return ONLY a JSON object with these fields:
...
- If there are runtime errors/exceptions, put the traceback in stderr.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (language === "python") {
      ...
      // No local Python → use LLM fallback
      const result = await executePythonViaLLM(code);
